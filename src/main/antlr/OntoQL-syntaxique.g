 ...
 */
class OntoQLBaseParser extends Parser;
...
statement
	: ( ddlStatement | updateStatement | deleteStatement | queryExpression | insertStatement )
	;
...
createStatement
	: CREATE^ (ddlExtent | viewDefinition | ontologyDefinition | entityDefinition)
	;
...
ontologyDefinitionBody
	: (classDefinitionBody)? | aPosterioriCaseOfDefinitionBody
	;

classDefinitionBody
	: OPEN! (descriptorClause)? (propertiesClause)? CLOSE!
	;
...
parameterStatement
	: 	SET^ (namepaceSpecification | languageSpecification)
	;
...
queryRule!
	: sf:selectFrom
	  ...
	  (p:preferenceClause)?
	  (n:namespaceClause)? {
		#queryRule = #([QUERY,"query"],n,sf,w,g,o,l,p);
	}
	;
...
identPrimary
	...
	| aggregate
	| TYPEOF^ OPEN! identifier CLOSE! ( DOT^ path )?
	;
...
propIdentifier
	: i:identifier (lb:OPEN_BRACKET^ (FR {#lb.setType(LANGUE_OP);} | EN {#lb.setType(LANGUE_OP);} | NUM_INT {#lb.setType(INDEX_OP);} )  CLOSE_BRACKET!)?
	;
 ...
 * @author Stephane Jean
 */
class OntoQLBaseLexer extends Lexer;

options {
	exportVocab=OntoQL;      // call the vocabulary "Ontoql"
	testLiterals = false;
