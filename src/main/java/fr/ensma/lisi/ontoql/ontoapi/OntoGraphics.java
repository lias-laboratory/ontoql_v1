 ...
 */
public class OntoGraphics extends OntoRoot {

	public OntoGraphics() {
	}

	public OntoGraphics(int oid, OntoQLSession s) {
		super(oid, s);
	}
	...
	public void load() {
		try {
			...
			OntoQLResultSet resultSet = stmt.executeQuery("SELECT #oid FROM #graphics WHERE " + whereClause);
			if (resultSet.next()) {
				setOid(resultSet.getInt(1));

				isLoaded = true;
			} else {
				throw new JOBDBCException("the instance of graphics with oid " + oid + " doesn't exist");
			}
