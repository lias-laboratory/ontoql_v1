...
/**
 * OntoDependentProperty.
 *
 * @author Stéphane JEAN
 */
public class OntoDependentProperty extends OntoProperty {

	protected Set dependsOn;

	protected boolean isLoadedDependsOn;

	public OntoDependentProperty() {
	}

	public OntoDependentProperty(int oid, OntoQLSession s) {
		super(oid, s);
	}

	public Set getDependsOn() {
		if (!isLoadedDependsOn) {
			loadDependsOn();
		}
		return dependsOn;
	}
	...
	public void loadDependsOn() {
		try {
			OntoQLStatement stmt = session.createOntoQLStatement();
			OntoQLResultSet resultSet = stmt
					.executeQuery("SELECT #dependsOn FROM #dependentProperty WHERE #oid = " + oid + "");
			if (resultSet.next()) {
				setDependsOn(resultSet.getSet(1));

				isLoadedDependsOn = true;
			} else {
				throw new JOBDBCException("the instance of dependentProperty with oid " + getOid() + " doesn't exist");
			}
	...
	public void load() {
		try {
			...
			OntoQLResultSet resultSet = stmt.executeQuery(
					"SELECT #code, #version, #oid, #name[en], #name[fr], #definition[en], #definition[fr], #revision, #note[en], #note[fr], #remark[en], #remark[fr], #dateOfCurrentVersion, #dateOfCurrentRevision, #dateOfOriginalDefinition, #shortName[en], #shortName[fr] FROM #dependentProperty WHERE "
							+ whereClause);
			if (resultSet.next()) {
				...
				throw new JOBDBCException("the instance of dependentProperty with oid " + oid + " doesn't exist");
			}
		} catch (SQLException e) {
			throw new JOBDBCException(e);
		}
