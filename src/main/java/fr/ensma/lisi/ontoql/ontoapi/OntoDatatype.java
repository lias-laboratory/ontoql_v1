 ...
 */
public class OntoDatatype extends OntoRoot {

	public OntoDatatype() {
	}

	public OntoDatatype(int oid, OntoQLSession s) {
		super(oid, s);
	}
	...
	public void load() {
		try {
			...
			OntoQLResultSet resultSet = stmt.executeQuery("SELECT #oid FROM #datatype WHERE " + whereClause);
			if (resultSet.next()) {
				setOid(resultSet.getInt(1));

				isLoaded = true;
			} else {
				throw new JOBDBCException("the instance of datatype with oid " + oid + " doesn't exist");
			}
