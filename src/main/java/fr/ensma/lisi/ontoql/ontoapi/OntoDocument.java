 ...
 */
public class OntoDocument extends OntoRoot {

	public OntoDocument() {
	}

	public OntoDocument(int oid, OntoQLSession s) {
		super(oid, s);
	}
	...
	public void load() {
		try {
			...
			OntoQLResultSet resultSet = stmt.executeQuery("SELECT #oid FROM #document WHERE " + whereClause);
			if (resultSet.next()) {
				setOid(resultSet.getInt(1));

				isLoaded = true;
			} else {
				throw new JOBDBCException("the instance of document with oid " + oid + " doesn't exist");
			}
