...
/**
 * OntoNonDependentProperty
 *
 * @author Stéphane JEAN
 */
public class OntoNonDependentProperty extends OntoProperty {

	public OntoNonDependentProperty() {
	}

	public OntoNonDependentProperty(int oid, OntoQLSession s) {
		super(oid, s);
	}
	...
	public void load() {
		try {
			...
			OntoQLResultSet resultSet = stmt.executeQuery(
					"SELECT #code, #version, #oid, #name[en], #name[fr], #definition[en], #definition[fr], #revision, #note[en], #note[fr], #remark[en], #remark[fr], #dateOfCurrentVersion, #dateOfCurrentRevision, #dateOfOriginalDefinition, #shortName[en], #shortName[fr] FROM #nonDependentProperty WHERE "
							+ whereClause);
			if (resultSet.next()) {
				...
				throw new JOBDBCException("the instance of nonDependentProperty with oid " + oid + " doesn't exist");
			}
