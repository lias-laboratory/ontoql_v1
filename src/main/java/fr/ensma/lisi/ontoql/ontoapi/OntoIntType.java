 ...
 */
public class OntoIntType extends OntoNumberType {

	public OntoIntType() {
	}

	public OntoIntType(int oid, OntoQLSession s) {
		super(oid, s);
	}
	...
	public void load() {
		try {
			...
			OntoQLResultSet resultSet = stmt.executeQuery("SELECT #oid FROM #intType WHERE " + whereClause);
			if (resultSet.next()) {
				setOid(resultSet.getInt(1));

				isLoaded = true;
			} else {
				throw new JOBDBCException("the instance of intType with oid " + oid + " doesn't exist");
			}
