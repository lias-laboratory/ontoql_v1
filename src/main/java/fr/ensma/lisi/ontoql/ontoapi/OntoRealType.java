 ...
 */
public class OntoRealType extends OntoNumberType {

	public OntoRealType() {
	}

	public OntoRealType(int oid, OntoQLSession s) {
		super(oid, s);
	}
	...
	public void load() {
		try {
			...
			OntoQLResultSet resultSet = stmt.executeQuery("SELECT #oid FROM #realType WHERE " + whereClause);
			if (resultSet.next()) {
				setOid(resultSet.getInt(1));

				isLoaded = true;
			} else {
				throw new JOBDBCException("the instance of realType with oid " + oid + " doesn't exist");
			}
