 ...
 */
public class OntoOntology extends OntoRoot {

	public OntoOntology() {
	}

	public OntoOntology(int oid, OntoQLSession s) {
		super(oid, s);
	}
	...
	public void load() {
		try {
			...
			OntoQLResultSet resultSet = stmt
					.executeQuery("SELECT #oid, #namespace FROM #ontology WHERE " + whereClause);
			if (resultSet.next()) {
				setOid(resultSet.getInt(1));
				setNamespace(resultSet.getString(2));

				isLoaded = true;
			} else {
				throw new JOBDBCException("the instance of ontology with oid " + oid + " doesn't exist");
			}
