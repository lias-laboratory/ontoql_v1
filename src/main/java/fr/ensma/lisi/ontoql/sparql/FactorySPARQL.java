...
public class FactorySPARQL {
	...
	public CategorySPARQL createCategory(String name, String variable, SPARQLOntoQLWalker walker) {
		CategorySPARQL res = null;
		if (name.startsWith(OntoQLHelper.PREFIX_NAMESPACE_RDF)) {
	...
	public DescriptionSPARQL createDescription(String name, String variable, SPARQLOntoQLWalker walker) {
		DescriptionSPARQL res = null;
		if (name.startsWith(OntoQLHelper.PREFIX_NAMESPACE_RDF)
				|| name.startsWith(OntoQLHelper.PREFIX_ONTOLOGYMODEL_ELEMENT)) {
